...
import SendMail from '../services/ForgotPassWordServices/SendEmailTest';
...
export const resetPasswords = async (
  ...
  const { email, tokenSenha, newPassword } = req.body;

  const resetPassword = await ResetPassword(email, tokenSenha, newPassword);
  if (!resetPassword) {
